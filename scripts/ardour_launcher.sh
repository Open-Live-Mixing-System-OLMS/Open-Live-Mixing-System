    ...
    case "$backend" in
        ...
        "alsa")
            ...
            if [ -n "$USB_AUDIO_DEVICE" ]; then
                jack_cmd="jackd -d alsa -r$JACK_SAMPLE_RATE -p$JACK_PERIOD_SIZE -d $USB_AUDIO_DEVICE"
                print_status "Using detected USB device: $USB_AUDIO_DEVICE"
            else
                # Fallback to first available card if detection failed
                jack_cmd="jackd -d alsa -r$JACK_SAMPLE_RATE -p$JACK_PERIOD_SIZE"
...
# Main execution
print_status "Phase 0: Cleanup existing sessions"
cleanup_existing_ardour_sessions
